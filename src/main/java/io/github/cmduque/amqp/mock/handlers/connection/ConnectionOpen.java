...
public class ConnectionOpen implements IMethodHandler<AMQImpl.Connection.Open> {
    @Override
    public void handleMethod(ClientHandler clientHandler, int channelNumber, AMQImpl.Connection.Open method) throws IOException {
        AMQImpl.Connection.OpenOk response = new AMQImpl.Connection.OpenOk(clientHandler.getServerConfig().getKnownHosts());
