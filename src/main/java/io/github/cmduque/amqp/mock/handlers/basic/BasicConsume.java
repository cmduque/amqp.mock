...
public class BasicConsume implements IMethodHandler<AMQImpl.Basic.Consume> {
    @Override
    public void handleMethod(ClientHandler clientHandler, int channelNumber, AMQImpl.Basic.Consume method) throws IOException {
        ServerConfig serverConfig = clientHandler.getServerConfig();
        AMQImpl.Basic.ConsumeOk response = new AMQImpl.Basic.ConsumeOk(serverConfig.getConsumerTag());
        writeMethod(clientHandler.getDataOutputStream(), channelNumber, response);

        clientHandler.registerQueueConsumer(channelNumber, method.getQueue());
    }
}