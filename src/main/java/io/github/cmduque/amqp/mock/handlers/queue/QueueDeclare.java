...
public class QueueDeclare implements IMethodHandler<AMQImpl.Queue.Declare> {
    @Override
    public void handleMethod(ClientHandler clientHandler, int channelNumber, AMQImpl.Queue.Declare method) throws IOException {
        AMQImpl.Queue.DeclareOk response = new AMQImpl.Queue.DeclareOk(method.getQueue(), 0, 0);
