...
public class BasicCancel implements IMethodHandler<AMQImpl.Basic.Cancel> {
    @Override
    public void handleMethod(ClientHandler clientHandler, int channelNumber, AMQImpl.Basic.Cancel method) throws IOException {
        ServerConfig serverConfig = clientHandler.getServerConfig();
        AMQImpl.Basic.CancelOk response = new AMQImpl.Basic.CancelOk(serverConfig.getConsumerTag());
