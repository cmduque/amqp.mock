...
public class TxCommit implements IMethodHandler<AMQImpl.Tx.Commit> {
    @Override
    public void handleMethod(ClientHandler clientHandler, int channelNumber, AMQImpl.Tx.Commit method) throws IOException {
        AMQImpl.Tx.CommitOk response = new AMQImpl.Tx.CommitOk();
