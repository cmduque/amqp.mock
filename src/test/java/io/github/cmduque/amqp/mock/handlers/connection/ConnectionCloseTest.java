...
@PrepareForTest({IOUtils.class, ConnectionClose.class})
public class ConnectionCloseTest {
    @Test
    public void handleMethodMustWriteConnectionCloseOkMethodAndStopClientHandler() throws Exception {
        int channelNumber = 2;
        ...
        AMQImpl.Connection.CloseOk response = mock(AMQImpl.Connection.CloseOk.class);
        whenNew(AMQImpl.Connection.CloseOk.class).withNoArguments().thenReturn(response);
        ConnectionClose methodHandler = new ConnectionClose();
