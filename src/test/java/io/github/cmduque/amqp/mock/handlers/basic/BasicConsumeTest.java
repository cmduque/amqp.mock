...
@PrepareForTest({IOUtils.class, BasicConsume.class})
public class BasicConsumeTest {
    @Test
    public void handleMethodMustWriteBasicConsumeOkMethodAndRegisterQueueConsumer() throws Exception {
        int channelNumber = 2;
        ...
        AMQImpl.Basic.Consume method = new AMQImpl.Basic.Consume(0, queueName, serverConfig.getConsumerTag(), false, false, false, false, null);
        when(clientHandler.getServerConfig()).thenReturn(serverConfig);
        mockStatic(IOUtils.class);
        AMQImpl.Basic.ConsumeOk response = mock(AMQImpl.Basic.ConsumeOk.class);
        whenNew(AMQImpl.Basic.ConsumeOk.class).withArguments(serverConfig.getConsumerTag()).thenReturn(response);
        BasicConsume methodHandler = new BasicConsume();
        ...
        verify(clientHandler, times(1)).registerQueueConsumer(channelNumber, queueName);
    }
}