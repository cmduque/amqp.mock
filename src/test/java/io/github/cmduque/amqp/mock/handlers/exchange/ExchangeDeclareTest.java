 ...
 */
package io.github.cmduque.amqp.mock.handlers.exchange;
...
@PrepareForTest({IOUtils.class, ExchangeDeclare.class})
public class ExchangeDeclareTest {
    @Test
    public void handleMethodMustWriteExchangeDeclareOkMethod() throws Exception {
        int channelNumber = 2;
        ...
        AMQImpl.Exchange.Declare method = mock(AMQImpl.Exchange.Declare.class);
        ServerConfig serverConfig = ServerConfig.defaultConfig();
        when(clientHandler.getServerConfig()).thenReturn(serverConfig);
        mockStatic(IOUtils.class);
        AMQImpl.Exchange.DeclareOk response = mock(AMQImpl.Exchange.DeclareOk.class);
        whenNew(AMQImpl.Exchange.DeclareOk.class).withNoArguments().thenReturn(response);
        ExchangeDeclare methodHandler = new ExchangeDeclare();
