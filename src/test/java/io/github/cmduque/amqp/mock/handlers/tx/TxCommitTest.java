...
@PrepareForTest({IOUtils.class, TxCommit.class})
public class TxCommitTest {
    @Test
    public void handleMethodMustWriteTxCommitOkMethod() throws Exception {
        int channelNumber = 2;
        ...
        AMQImpl.Tx.CommitOk response = mock(AMQImpl.Tx.CommitOk.class);
        whenNew(AMQImpl.Tx.CommitOk.class).withNoArguments().thenReturn(response);
        TxCommit methodHandler = new TxCommit();
